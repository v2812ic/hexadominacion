...
def interacciones(estado, jugadores, acciones):
        ...
        for jugador, datosJugador in jugadores.items(): # Desempaqueta el jugador que realiza la accion
            if accion in acciones[jugador]: # Comprueba si decide realizarla
                ...
                #Accion de mover la capital
                    ...
                    madera = 30
                    metal = 20
                    if estado[origen]["propietario"] == jugador and estado[destino]["propietario"] == jugador and estado[destino]["nivel"] >= 3 and cumpleRequisitos(datosJugador, trigo, piedra, madera, metal) and estado[destino]["recurso"] == "Ciudad":
    ...
    for territorio, datosTerritorio in estado.items():
        ...
        # DLC de revolución industrial
        if recurso != "Ciudad" and recurso != "Capital":
            nivel = min(nivel, datosTerritorio["soldados"])

        if propietario in jugadores:
            if recurso != "Ciudad" and recurso != "Capital":

                if nivel == 1:
                    recibe = random.randint(4, 6)*5
                elif nivel == 2:
                    recibe = random.randint(6, 10)*5
                elif nivel == 3:
                    recibe = random.randint(8, 12)*5
                elif nivel == 4:
                    recibe = random.randint(12, 16)*5
                elif nivel == 5:
                    recibe = random.randint(18, 23)*5
                elif nivel == 0:
                    recibe = 0
                
                # Fin DLC de la rebelión industrial
        ...
        if "Cesion" in acciones[jugador]:
            ...
            for cesion in cesiones:
                ...
                if estado[cedido]["propietario"] == jugador and not jugadores[destinatario]["Eliminado"]:
                    ...
                    if estado[cedido]["recurso"] == "Capital":
                        estado[cedido]["recurso"] = "Ciudad"
